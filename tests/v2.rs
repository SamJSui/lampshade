...
async fn fixed_key_value_sort_uses_the_adapter_path_at_nonzero_offsets() {
    ...
    let input_buffer = initialized_key_values_at(&context.device, "v2 fixed KV input", &records);
    let output_buffer = empty_key_value_buffer(&context.device, "v2 fixed KV output");
    let input = GpuSlice::from_range(&input_buffer, VIEW_START..VIEW_START + capacity).unwrap();
    let output =
...
async fn counted_key_value_sort_clamps_count_to_capacity() {
    ...
    let capacity = records.len() as u32;
    let input_buffer = initialized_key_values_at(&context.device, "v2 counted KV input", &records);
    let output_buffer = empty_key_value_buffer(&context.device, "v2 counted KV output");
    let mut counts = vec![0_u32; BUFFER_ITEMS];
    counts[VIEW_START as usize] = capacity + 100;
    let count_buffer = context
        .device
        .create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("v2 oversized count"),
    ...
    let count = match input.extent() {
        wgpu_primitives::v2::Extent::Gpu(count) => count,
        wgpu_primitives::v2::Extent::Fixed(_) => unreachable!(),
    };
    let mut primitives = Primitives::from_context(&context);
    primitives
...
async fn typed_key_value_views_validate_capacity_usage_and_aliases() {
    ...
    let input_buffer =
        initialized_key_values_at(&context.device, "v2 KV validation input", &records);
    let mask_buffer = empty_buffer(&context.device, "v2 KV validation mask");
    let output_buffer = empty_key_value_buffer(&context.device, "v2 KV validation output");
    let count_buffer = empty_buffer(&context.device, "v2 KV validation count");
    let missing_copy_src = context.device.create_buffer(&wgpu::BufferDescriptor {
        label: Some("v2 mask missing COPY_SRC"),
...
async fn run_key_value_pipeline(
    context: &wgpu_primitives::Context,
    records: &[KeyValue],
    predicate: U32Predicate,
) -> Vec<KeyValue> {
    let capacity = records.len() as u32;
    let input_buffer = initialized_key_values_at(&context.device, "v2 particle input", records);
    let mask_buffer = empty_buffer(&context.device, "v2 particle mask");
    let compacted_buffer = empty_key_value_buffer(&context.device, "v2 particle compacted");
    let sorted_buffer = empty_key_value_buffer(&context.device, "v2 particle sorted");
    let count_buffer = empty_buffer(&context.device, "v2 particle count");
    let readback_size = 8 + u64::from(capacity) * size_of::<KeyValue>() as u64;
    let readback = context.device.create_buffer(&wgpu::BufferDescriptor {
        label: Some("v2 particle readback"),
        size: readback_size,
